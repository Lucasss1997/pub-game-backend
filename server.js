...
function parsePoundsToCents(input) {
  ...
  if (s === '' || s === '.') return 0;
  if (!/^\d+(\.\d{0,2})?$/.test(s)) {
    throw Object.assign(new Error('Invalid money format'), { status: 400 });
  }
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO users (email, password_hash, pub_name) VALUES ($1, $2, $3) RETURNING id, pub_name',
      [email, hashedPassword, pubName]
    );
    res.json({ id: result.rows[0].id, pub_name: result.rows[0].pub_name });
  } catch (err) {
